...
profile(model, inputs=(example_input,))
...
model = VariableMLP(nlayer=1, output_size=10, input_size=28*28, hidden_size=32)
model = model.to(device)
...
model.to("cpu")
example_input = next(iter(test_loader))[0][0].to("cpu").reshape([1,-1])
from utils.export import export
export(model, input_array=example_input, onnx_filename="MNIST/MLP_network.onnx", input_filename="MNIST/MLP_input.json")
...
os.system(f"ezkl setup -M MNIST/MLP_network.ezkl --srs-path={SRS_PATH % logrows} --vk-path=MNIST/MLP_vk.key --pk-path=MNIST/MLP_pk.key" + pipstd('setup'))

# %% 4.1 Make the data for inference
print("Saving real data for ezkl inference")
os.makedirs('MNIST/data/mlp_ezkl_inputs', exist_ok=True)
os.makedirs('MNIST/data/mlp_ezkl_witnesses', exist_ok=True)
os.makedirs('MNIST/data/mlp_ezkl_proofs', exist_ok=True)
...
for i, (image, label) in enumerate(tqdm(test_loader, desc="Exporting data")):
    ...
    input_filename = f'MNIST/data/mlp_ezkl_inputs/input_{i}_lab_{label.item()}.json'
    json.dump( data, open( input_filename, 'w' ) )
    if i ==1000: break

# %% 4.2 Loop and prove
print("Proving over real data")
for input_file in tqdm(glob.glob("MNIST/data/mlp_ezkl_inputs/*.json")):
    proof_path = f"MNIST/data/mlp_ezkl_proofs/MLP_MNIST{input_file.split('input_')[-1][:-5]}.proof"
    witness_path = f"MNIST/data/mlp_ezkl_witnesses/MLP_{input_file.split('input_')[-1][:-5]}.json"
    os.system(f"ezkl gen-witness -M MNIST/MLP_network.ezkl --data {input_file} --output {witness_path}" + pipstd('prove'))
    res = os.system(f"ezkl prove -M MNIST/MLP_network.ezkl --witness {witness_path} --pk-path=MNIST/MLP_pk.key --proof-path={proof_path} --srs-path={SRS_PATH % logrows}" + pipstd('prove'))
    # if res!=0: print(f"Prove error on {input_file.split('input_')[-1][:-5]}, error {res}")
    break

# %% 4.3 Quickly confirm one of these proofs verifies
proof_path = glob.glob("MNIST/data/mlp_ezkl_proofs/*.proof")[0]
os.system(f"ezkl verify --settings-path MNIST/MLP_settings.json --proof-path {proof_path} --vk-path MNIST/MLP_vk.key --srs-path {SRS_PATH % logrows}")
